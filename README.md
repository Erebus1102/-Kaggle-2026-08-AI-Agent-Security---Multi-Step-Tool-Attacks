...
1. Kaggle 下载包中的实际评测代码和 fixture 数据
2. 官方 starter notebook
3. JED 官方文档、上游仓库与 PyPI 元数据
4. 本 README 的解释性总结

Kaggle 规则、时间线、奖项和 Hosted 环境变量可能继续更新，提交前应再次核对竞赛原页。代码常量、数据数量和评测路径均以当前 3.1.2 下载快照为准。
...
3. [可信评测链](#3-可信评测链)
4. [快速开始](#4-快速开始)
5. [Agent 工具箱](#5-agent-工具箱)
6. [数据与攻击语料](#6-数据与攻击语料)
7. [环境 API](#7-环境-api)
8. [四个安全谓词](#8-四个安全谓词)
9. [评分公式与计分 Cell](#9-评分公式与计分-cell)
10. [目标模型与 Guardrail](#10-目标模型与-guardrail)
11. [硬性约束与预算](#11-硬性约束与预算)
12. [源码揭示的关键行为](#12-源码揭示的关键行为)
13. [Starter 与内置基线](#13-starter-与内置基线)
14. [本地验证与证据边界](#14-本地验证与证据边界)
15. [目录结构](#15-目录结构)
16. [时间线、奖项与引用](#16-时间线奖项与引用)
17. [术语表](#17-术语表)
...
2. 回放时真实发生的工具事件，包括参数、来源、副作用、成功状态和输出
3. 由可信评测端重新计算的安全谓词和计分 Cell
...
| 工具 | side effect | scope | 实际行为 |
|---|---|---|---|
| `web.search` | NETWORK | public | 在离线网页正文和标题中做子串检索，最多返回 10 条；无命中时回退到 `page_0` |
| `web.open` | NETWORK | public | 按已知页面 ID 读取正文 |
| `fs.read` | READ | local | 读取沙箱内 UTF-8 文件 |
| `fs.write` | WRITE | local | 创建或覆盖沙箱文件 |
| `fs.delete` | WRITE | local | 删除沙箱文件 |
| `shell.run` | EXEC | local | 只允许 `echo`、`ls`、`pwd`、`whoami`、`date` |
| `email.list` | READ | local | 分页列出邮件浅摘要，单页上限 50 |
| `email.read` | READ | local | 按消息 ID 读取完整邮件 |
| `email.send` | SHARE | external | 把外发邮件写入模拟 sent 邮箱 |
| `http.post` | SHARE | external | 把 URL 和 data 写入模拟 egress sink；私网目标会被拒绝 |
